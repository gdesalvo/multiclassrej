...
function stratifiedreject3(input_file)
            ...
            for ii=1:length(d_vec)
                for kkk=1:length(d_vec)               
                  if d_vec(ii)<=3 && d_vec(kkk)<=3
                    ...
                    fprintf(fileID,'%s %f %s %f  %s %f %s %f %s %f %s %f %s %f %f %f %f %s %f %f %f %f %s %f  %f %f %f %s %f %f %f %f %s %f %f %f %f %s %f %f %f %f \n','cost=',c_vec(i),',eraserun=',eraserun,',d1=',d_vec(ii),',d2=',d_vec(kkk),',lam1=',lam1_vec(j),',lam2=',lam2_vec(k),',train=',train(1,:),'train_sd=',train(2,:) ,',val=',val(1,:),',val_sd=',val(2,:),',test=',test(1,:), ',test_sd=',test(2,:));
                    ...
                    for ss=1:length(ssigma)
                       ...
                       fprintf(fileID,'%s %f %s %f %s %f %s %f %s %f %s %f %s %f %f %f %f %s %f %f %f %f %s %f  %f %f %f %s %f %f %f %f %s %f %f %f %f %s %f %f %f %f \n','cost=',c_vec(i),',eraserun=',eraserun,',d1=',d_vec(ii),',d2=',d_vec(kkk),',sigma=',ssigma(ss),',lam1=',lam1_vec(j),',lam2=',lam2_vec(k),',train=',train(1,:),'train_sd=',train(2,:) ,',val=',val(1,:),',val_sd=',val(2,:),',test=',test(1,:), ',test_sd=',test(2,:));
                         if eraserun==1
                            numremovedruns=numremovedruns+1;
                        end
...
function [alpha,beta1]=genalg_dual(K1,K2,lam1,lam2,train_label,c)
    ...
    maximize(-lam2*quad_form((-alpha1+b1.*(alpha1 + alpha2 + alpha3) )', K1) -lam2*quad_form((-alpha2+b2.*(alpha1+alpha2+alpha3) )', K1) -lam2*quad_form((-alpha3+b3.*(alpha1+alpha2+alpha3 ) )', K1) - 4*lam1*quad_form(c*beta1-0.5*v1.*alpha1-0.5*v2.*alpha2-0.5*v3.*alpha3, K2)+ 8*lams*c*sum(beta1)+ 8*lams*sum(v1.*alpha1)+ 8*lams*sum(v2.*alpha2)+ 8*lams*sum(v3.*alpha3))
    subject to
...
function [minindex,throwout]=func_alpha(alpha,data,data_label,x,sigma,d1) %% w*x from alpha1 values 
    ...
    for col=1:size(data,2) 
          out(1)= out(1)+ ( -alpha(1,col) + (data_label(col)==1)*(alpha(1,col)+alpha(2,col)+alpha(3,col) ) ) *poly_kernel(data(:,col),x,d1,sigma);   
    end

    for col=1:size(data,2) 
          out(2)= out(2)+ ( -alpha(2,col) + (data_label(col)==2)*(alpha(1,col)+alpha(2,col)+alpha(3,col)) ) *poly_kernel(data(:,col),x,d1,sigma);   
    end
    
    for col=1:size(data,2)  
          out(3)= out(3)+ ( -alpha(3,col) + (data_label(col)==3)*(alpha(1,col)+alpha(2,col)+alpha(3,col) ) ) *poly_kernel(data(:,col),x,d1,sigma);   
    end
...
% FOR POLY KERNEL=2
    out=0;
    for col=1:size(data,2)
          out= out+ ( c*beta1(col)-0.5*(data_label(col)~=1)*alpha(1,col)-0.5*(data_label(col)~=2)*alpha(2,col)-0.5*(data_label(col)~=3)*alpha(3,col) )*poly_kernel(data(:,col),x,d,sigma);  
    end
